...
>(({ className, ...props }, ref) => {
  ...
  return (
    <Button
      ref={ref}
      className={cn(
        "absolute rounded-full bg-white w-9 h-9 cursor-pointer",
        orientation === "horizontal"
...
const CarouselDots = () => {
  ...
  return (
    <div className="flex items-center justify-center space-x-2 transform translate-y-2.5">
      {Array.from({ length: slidesCount }).map((_, index) => (
        <Button
          ...
          className={cn(
            ...
            selectedIndex === index
              ? "bg-scarlet-red w-2.5 h-2.5"
              : "bg-medium-light-gray w-3 h-3"
          )}
